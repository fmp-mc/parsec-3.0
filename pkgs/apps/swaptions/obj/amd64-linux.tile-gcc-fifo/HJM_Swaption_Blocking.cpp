...
PROFILER_TIMER_INIT(0);
  ...
  FTYPE **ppdDrifts; 
  FTYPE *pdTotalDrift;
PROFILER_ADD_TIME(PROFILE_WORK, PROFILER_FORK_TID(tid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, PROFILER_FORK_TID(tid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_ADD_TIME(PROFILE_WORK, PROFILER_FORK_TID(tid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
  ...
  for (l=0;l<=lTrials-1;l+=BLOCKSIZE) {
      ...
      iSuccess = HJM_SimPath_Forward_Blocking(ppdHJMPath, iN, iFactors, dYears, pdForward, pdTotalDrift,ppdFactors, &iRndSeed, BLOCKSIZE, tid); /* GC: 51% of the time goes here */
       if (iSuccess!=1)
	return iSuccess;
...
PROFILER_ADD_TIME(PROFILE_WORK, PROFILER_FORK_TID(tid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
      iSuccess = Discount_Factors_Blocking(pdPayoffDiscountFactors, iN, dYears, pdDiscountingRatePath, BLOCKSIZE, tid); /* 15% of the time goes here */
...
PROFILER_ADD_TIME(PROFILE_WORK, PROFILER_FORK_TID(tid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
      iSuccess = Discount_Factors_Blocking(pdSwapDiscountFactors, iSwapVectorLength, dSwapVectorYears, pdSwapRatePath, BLOCKSIZE, tid);
      if (iSuccess!=1)
	return iSuccess;
