...
#if !defined(BUILD_PARSEC_FOR_LINUX)
#else
#define _GNU_SOURCE
#include <sched.h>
#endif
...
#define DEFINE_PROFILE_DATA_HERE
#include <parsec_profiler.h>
...
static void write_chunk_to_file(int fd, chunk_t *chunk, const int tid) {
PROFILER_TIMER_INIT(0);
  assert(chunk!=NULL);
...
PROFILER_ADD_TIME(PROFILE_SYNC, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_TIMER_UPDATE(0);
    write_file(fd, TYPE_COMPRESS, chunk->compressed_data.n, chunk->compressed_data.ptr);
PROFILER_ADD_TIME(PROFILE_FILE_IO, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
PROFILER_TIMER_UPDATE(0);
    mbuffer_free(&chunk->compressed_data);
    chunk->header.state = CHUNK_STATE_FLUSHED;
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
  } else {
PROFILER_TIMER_UPDATE(0);
 ...
 */
void sub_Compress(chunk_t *chunk, const int tid) {
    size_t n;
    int r;
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
        break;
#ifdef ENABLE_GZIP_COMPRESSION
      case COMPRESS_GZIP:
PROFILER_TIMER_UPDATE(0);
...
void *Compress(void * targs) {
  ...
  const int tid = 2 + conf->nthreads * 2 + args->tid;
  chunk_t * chunk;
  int r;
...
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
  ...
  while(1) {
    ...
    if (ringbuffer_isEmpty(&recv_buf)) {
      r = queue_dequeue(&compress_que[qid], &recv_buf, ITEM_PER_FETCH, tid);
      if (r < 0) break;
    }
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
    ...
    if (ringbuffer_isFull(&send_buf)) {
      r = queue_enqueue(&reorder_que[qid], &send_buf, ITEM_PER_INSERT, tid);
      assert(r>=1);
    }
  }
 ...
 */
int sub_Deduplicate(chunk_t *chunk, const int tid) {
PROFILER_TIMER_INIT(0);
  int isDuplicate;
  chunk_t *entry;
...
void * Deduplicate(void * targs) {
  struct thread_args *args = (struct thread_args *)targs;
  const int qid = args->tid / MAX_THREADS_PER_QUEUE;
  const int tid = 2 + conf->nthreads + args->tid;
  chunk_t *chunk;
  int r;
...
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
  ...
  while (1) {
    ...
    if (ringbuffer_isEmpty(&recv_buf)) {
      r = queue_dequeue(&deduplicate_que[qid], &recv_buf, CHUNK_ANCHOR_PER_FETCH, tid);
      if (r < 0) break;
    }
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
    ...
    //Do the processing
    int isDuplicate = sub_Deduplicate(chunk, tid);
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
      if (ringbuffer_isFull(&send_buf_compress)) {
        r = queue_enqueue(&compress_que[qid], &send_buf_compress, ITEM_PER_INSERT, tid);
        assert(r>=1);
      }
    } else {
PROFILER_TIMER_UPDATE(0);
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
      if (ringbuffer_isFull(&send_buf_reorder)) {
        r = queue_enqueue(&reorder_que[qid], &send_buf_reorder, ITEM_PER_INSERT, tid);
        assert(r>=1);
      }
    }
  }
...
void *FragmentRefine(void * targs) {
  struct thread_args *args = (struct thread_args *)targs;
  const int qid = args->tid / MAX_THREADS_PER_QUEUE;
  const int tid = 2 + args->tid; // tid for profiling
  ringbuffer_t recv_buf, send_buf;
  int r;
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
  ...
  while (TRUE) {
    ...
    if (ringbuffer_isEmpty(&recv_buf)) {
      r = queue_dequeue(&refine_que[qid], &recv_buf, MAX_PER_FETCH, tid);
      if (r < 0) {
        break;
      }
    }
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
        ...
        r = mbuffer_split(&chunk->uncompressed_data, &temp->uncompressed_data, offset, tid);
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
        if (ringbuffer_isFull(&send_buf)) {
          r = queue_enqueue(&deduplicate_que[qid], &send_buf, CHUNK_ANCHOR_PER_INSERT, tid);
          assert(r>=1);
        }
PROFILER_TIMER_UPDATE(0);
...
PROFILER_TIMER_INIT(0);
  ...
  int r;
PROFILER_ADD_TIME(PROFILE_WORK, 1, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_TIMER_UPDATE(0);
  ...
  assert(r==0);
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, 1, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_ADD_TIME(PROFILE_FILE_IO, 1, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
    }
    //No data left over from last iteration and also nothing new read in, simply clean up and quit
    if(bytes_left + bytes_read == 0) {
PROFILER_TIMER_UPDATE(0);
...
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, 1, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
          ...
          //split it into two pieces
          r = mbuffer_split(&chunk->uncompressed_data, &temp->uncompressed_data, offset + ANCHOR_JUMP, 1);
          if(r!=0) EXIT_TRACE("Unable to split memory buffer.\n");
PROFILER_TIMER_UPDATE(0);
          temp->header.state = CHUNK_STATE_UNCOMPRESSED;
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
          ...
          if(ringbuffer_isFull(&send_buf)) {
            r = queue_enqueue(&refine_que[qid], &send_buf, ANCHOR_DATA_PER_INSERT, tid);
            assert(r>=1);
...
void *Reorder(void * targs) {
  struct thread_args *args = (struct thread_args *)targs;
  int qid = 0;
  const int tid = 2 + conf->nthreads * 3;
  int fd = 0;
  ...
  while(1) {
    ...
    if (ringbuffer_isEmpty(&recv_buf)) {
      ...
      for(i=0,r=0; r<=0 && i<args->nqueues; i++) {
        r = queue_dequeue(&reorder_que[qid], &recv_buf, ITEM_PER_FETCH, tid);
        qid = (qid+1) % args->nqueues;
      }
      if(r<0) break;
    }
...
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
        Insert(chunk, tele.queue, tid);
PROFILER_TIMER_UPDATE(0);
        T = TreeInsert(tele, T);
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
      } else {
        Insert(chunk, pos->Element.queue, tid);
      }
      continue;
    }
...
PROFILER_ADD_TIME(PROFILE_WORK, tid, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
    do {
      write_chunk_to_file(fd, chunk, tid);
      if(chunk->header.isDuplicate) {
PROFILER_TIMER_UPDATE(0);
        free(chunk);
        chunk=NULL;
...
#ifdef ENABLE_STATISTICS
  init_stats(&stats);
PROFILER_ADD_TIME(PROFILE_WORK_SERIAL, 0, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
#endif
...
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, 0, PROFILER_TIMER_NOW, PROFILER_TIMER(0));

PROFILER_TIMER_UPDATE(0);
  /* src file stat */
  if (stat(conf->infile, &filestat) < 0) 
      EXIT_TRACE("stat() %s failed: %s\n", conf->infile, strerror(errno));
PROFILER_ADD_TIME(PROFILE_FILE_IO, 0, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_NEW_FORK(1);
  ...
  // Set policy and priority for main thread
  ...
  // Set attribute for worker threads
  pthread_attr_t *attrs = malloc(sizeof(pthread_attr_t)*nthreads);
  ...
  pthread_create(&threads_process, 
#if defined(USE_SCHED_FIFO)
          &attrs[cur_attr_idx],
#else
          NULL,
...
PROFILER_NEW_FORK(conf->nthreads);
  ...
  for (i = 0; i < conf->nthreads; i ++) {
     ...
     pthread_create(&threads_anchor[i], 
#if defined(USE_SCHED_FIFO)
          &attrs[(cur_attr_idx + i) % nthreads],
#else
          NULL,
#endif
...
PROFILER_NEW_FORK(conf->nthreads);
  ...
  for (i = 0; i < conf->nthreads; i ++) {
    ...
    pthread_create(&threads_chunk[i], 
#if defined(USE_SCHED_FIFO)
          &attrs[(cur_attr_idx + i) % nthreads],
#else
          NULL,
...
PROFILER_NEW_FORK(conf->nthreads);
  ...
  for (i = 0; i < conf->nthreads; i ++) {
    ...
    pthread_create(&threads_compress[i], 
#if defined(USE_SCHED_FIFO)
          &attrs[(cur_attr_idx + i) % nthreads],
#else
          NULL,
...
PROFILER_NEW_FORK(1);
  ...
  pthread_create(&threads_send, 
#if defined(USE_SCHED_FIFO)
          &attrs[(cur_attr_idx + 0) % nthreads],
#else
          NULL,
#endif
  ...
  cur_attr_idx += 1;
#endif
PROFILER_ADD_TIME(PROFILE_SYNC, 0, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_ADD_TIME(PROFILE_WORK, 0, PROFILER_TIMER_NOW, PROFILER_TIMER(0));
PROFILER_TIMER_UPDATE(0);
...
static char test[sizeof(chunk_t)-256];
