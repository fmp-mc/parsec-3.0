...
namespace std { // TODO: ertl-liyixiao

#define INT_MAX ((1UL << 32) - 1)
#undef ferror
int ferror(FILE *stream) {
	return __sferror(stream);
}
...
#endif
PROFILER_ADD_TIME(PROFILE_SYNC, PROFILER_FORK_TID(pid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
PROFILER_TIMER_UPDATE(0);
  ...
  static double* costs; //cost for each thread. 
  static int i;
PROFILER_ADD_TIME(PROFILE_WORK, PROFILER_FORK_TID(pid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
...
PROFILER_ADD_TIME(PROFILE_MALLOCATOR, PROFILER_FORK_TID(pid), PROFILER_TIMER_NOW, PROFILER_TIMER(0));
  }
...
PROFILER_NEW_FORK(nproc);
  ...
  pthread_attr_t *attrs = new pthread_attr_t[nthreads];
